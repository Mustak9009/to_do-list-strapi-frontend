...
export default function Home({todos}:{todos:ToDoType}) {
  ...
  async function createTodo(){
    ...
    `;
    if(todoInput != ''){
      const data = await dataTunnel(createTodo,{title:todoInput,complete:false});
  ...
  async function delteTodo(id:string) { //$id:ID! type -> according Graphql rule
    const delteTodo = gql`
          ...
          <div className="min-h-screen  flex justify-center items-center">
              <div className="p-2 bg-white rounded-sm">
                  <header className="m-2">
                    <h1 className="font-display text-3xl tracking-wide text-gray-700">ToDo App 😊</h1>
                    ...
                    <p className='text-red-500 font-medium'>{inputEmpty && 'Please add some text in input box..'}</p>
                  </header>
                  <section className="mb-5 mx-2 overflow-hidden">
                    {newTodos.length !=0? newTodos.map((item:ToDo)=>(
                      <div key={item.id} className="flex justify-between items-center bg-gray-100 rounded-sm  my-3 hover:scale-105 cursor-pointer">
                          <p className="font-medium p-2 ml-2">{item.attributes.title}</p>
                          ...
                          <p className='text-gray-500 font-medium'>Please add your toTo item...😉</p>
                      </div>}
                  </section>
                 {newTodos.length != 0 && 
                  <footer>
